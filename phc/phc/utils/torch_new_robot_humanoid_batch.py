...
# TODO: 添加并修改一些ROTATION_AXIS
NEW_ROBOT_ROTATION_AXIS = torch.tensor([[
    [0, 0, 1], # l_hip_yaw
    [0, 1, 0], # l_hip_roll
    [1, 0, 0], # l_hip_pitch
...
class New_Robot_Humanoid_Batch:

    def __init__(self, mjcf_file = f"resources/robots/h1_2/h1_2.xml", extend_hand = True, extend_head = False, device = torch.device("cpu")):   # TODO：修改为新的机器人模型文件
        ...
        if extend_hand:
            ...
            self._parents = torch.cat((mjcf_data['parent_indices'], torch.tensor([20, 27]))).to(device) # Adding the hands joints   # TODO: 修改为新的机器人模型的父节点
            arm_length = 0.05    # TODO: 修改为新的机器人模型的手臂长度
    ...
    def fk_batch(self, pose, trans, convert_to_mat=True, return_full = False, dt=1/30):
        ...
        if self.extend_hand and self.extend_head and pose.shape[-2] == 30:  # TODO：修改为新的机器人模型的关节数
